...
export interface RemoveStaffByOrganizerDto {
  callerRef: string;  // organizador — viene del JWT
  accountId: string;  // account STAFF a desactivar
}

export class RemoveStaffByOrganizerUseCase {
  constructor(
    private readonly repo: AccountRepository,
    private readonly eventRepo: AccountEventRepository,
  ) {}

  async execute(dto: RemoveStaffByOrganizerDto): Promise<void> {
    ...
    if (!account.getType().isStaff()) throw new InsufficientPermissionsError();

    account.deactivate();
    await this.repo.save(account);

    this.eventRepo.save({ accountId: account.getId(), type: 'deactivated', performedBy: callerRef })
      .catch(err => logger.warn({ err }, '[RemoveStaffByOrganizer] event error'));
  }
}
